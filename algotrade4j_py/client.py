...
class MarketDataClient:
    ...
    def __init__(self, api_key=None, api_url="https://api.algotrade4j.trade/api/v1/marketdata/candles"):
        """
        ...
        if not self.api_key:
            raise ValueError("API key must be provided either directly or via the environment variable 'MARKETDATA_API_KEY'.")
    ...
    def get_candles(self, instrument, broker, from_date, to_date, period):
        ...
        Returns:
            pandas.DataFrame: A DataFrame containing the candle data.
        ...
        if broker not in self.SUPPORTED_BROKERS:
            raise ValueError(f"Broker '{broker}' is not supported. Supported brokers: {self.SUPPORTED_BROKERS}")

        if instrument not in self.SUPPORTED_INSTRUMENTS:
            raise ValueError(f"Instrument '{instrument}' is not supported. Supported instruments: {self.SUPPORTED_INSTRUMENTS}")

        if period not in self.SUPPORTED_PERIODS:
            raise ValueError(f"Period '{period}' is not supported. Supported periods: {self.SUPPORTED_PERIODS}")
